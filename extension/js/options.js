...
chrome.storage.local.get('mainMemory', function (details) {
    ...
    for (var i = 1; i < allUrls.length; i++) {
        ...
        // allUrlsNode += '<div style="border-bottom:1px solid black;margin:5px;padding-bottom:10px;max-width:100%;">' +
        //     '<div style="font-size:15px;"><span style="padding-right:5px;background-color:red;color:white;border-radius:5px;padding:5px;width:90%">'
        //     +parseInt(allUrls[i].time)/60 +'mins</span><b>'+ allUrls[i].url + ' </b></div>' + 
        //     '<div style="width:10%"><button id="' + allUrls[i].url + '" class="btn btn-danger" style="position:relative;margin-top:2px;margin-right:2px;">Block</button></div></div>';
